...
CAN_CheckType CAN_Init(void)
{
    ...
    for(LoopIndex = 0; (LoopIndex < CAN_GROUPS_NUMBER) && (ErrorFlag == 0); LoopIndex ++)
    {
        if(CAN_ConfigParam[LoopIndex].CANId < CAN_NUMBER)
        {
            ...
            if(CAN_ConfigParam[LoopIndex].SelfTestEnable == 1)
            {
                CAN_SelfTest = CAN_SelfTestEnaled;
            }
            else
            {
                CAN_SelfTest = CAN_SelfTestDisabled;
            }
...
CAN_CheckType CAN_TxData(uint8_t ChannelID, uint8_t *DataPtr, uint8_t Module)
{
    ...
    if(CAN_ConfigParam[ChannelID].CANId < CAN_NUMBER)
    {
        ...
        CAN_ConfigParam[ChannelID].CAN_CallBackTxDone();
        RetVar = CAN_OK;
    }
    else
    {
...
CAN_CheckType CAN_RxData(uint8_t ChannelID, uint16_t* DataPtr, uint8_t Module)
{
    ...
    if(CAN_ConfigParam[ChannelID].CANId < CAN_NUMBER)
    {
        ...
        *(DataPtr + 3) = CANIF2DA1(CAN_ConfigParam[ChannelID].CANId);
        ...
        CAN_ConfigParam[ChannelID].CAN_CallBackRxDone();
        RetVar = CAN_OK;
    }
    else
    {
...
void CAN_SelfTestFunc(uint8_t ChannelID, uint8_t Module)
{
    ...
    {
        /* R is an abrreviation for Ready and we are asking if the another uc is Ready or not */
        CAN_TxData(ChannelID, "R",Module);
    }
    if(CAN_SelfTest == CAN_SelfTestTxDone)
    {
        CAN_RxData(ChannelID, &DataRxSelfTestPtr, Module);
    }
    if(CAN_SelfTest == CAN_SelfTestRxDone)
    {
        CAN_SelfTestDone();
    }
}
